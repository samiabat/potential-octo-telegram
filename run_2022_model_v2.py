"""Entry point — ICT 2022 Model backtest on 1m_data_2.csv.

Same strategy as run_2022_model.py — no changes to parameters.
Additionally generates one candlestick chart per trade showing:
  - 1m price context from the liquidity sweep through to exit
  - Vertical lines: 15m sweep (purple), 15m MSS (orange), entry (blue)
  - Horizontal lines: entry (blue), stop (red), target (green)
  - Shaded trade zone (green=win, red=loss, gold=expired)

Charts saved to  results_2022_model_v2/trade_charts/

Usage:
    python3 run_2022_model_v2.py
"""
...
RESULTS_DIR = Path("results_2022_model_v2")
...
def main() -> None:
    print("=" * 60)
    print(" ICT 2022 Model v2 — Dataset: 1m_data_2.csv")
    print("=" * 60)
    ...
    # ------------------------------------------------------------------
    # Run the model  (identical config to run_2022_model.py)
    # ------------------------------------------------------------------
    print("\nRunning 2022 Model (HTF bias → 15m sweep → 15m MSS → 1m FVG)...")
    ...
    if by_kz   is not None: by_kz.to_csv(RESULTS_DIR / "stats_by_killzone.csv")
    if by_type is not None: by_type.to_csv(RESULTS_DIR / "stats_by_entry_type.csv")
    if by_dir  is not None: by_dir.to_csv(RESULTS_DIR / "stats_by_direction.csv")

    # ------------------------------------------------------------------
    # Summary + breakdown plots
    # ------------------------------------------------------------------
    print(f"\nGenerating summary charts → {RESULTS_DIR}/")
    ...
    # ------------------------------------------------------------------
    print(f"\nGenerating per-trade charts → {CHARTS_DIR}/")
    plot_all_trade_charts(
        trades, df1m, CHARTS_DIR,
        context_bars_before=60,   # ~60 min before sweep
        context_bars_after=15,    # ~15 min after exit
    )

    print("\nDone.")
    print(f"Results  → {RESULTS_DIR}/")
    print(f"Charts   → {CHARTS_DIR}/  ({len(trades)} files)")
