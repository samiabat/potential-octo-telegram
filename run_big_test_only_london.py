"""2022 Model backtest — full 1m_data.csv (2016-2025), London session only.

Runs the ICT 2022 Model on the full dataset with the London killzone filter
(03:00–05:00 ET) and writes all results to big_test_only_london/.

Usage:
    python3 run_big_test_only_london.py
"""
...
RESULTS_DIR = Path("big_test_only_london")
...
def main() -> None:
    print("=" * 65)
    print(" ICT 2022 Model — Full Dataset (2016-2025)  |  London Session Only")
    print("=" * 65)
    ...
    # ------------------------------------------------------------------
    # Run model — London session only
    # ------------------------------------------------------------------
    print("\nRunning 2022 Model (London killzone only: 09:30–12:00 ET) …")
    trades, diag = run_2022_model(
        ...
        use_fvg=True,
        use_killzones=True,
        allowed_killzones={"london"},   # London only
    )
    ...
    print("\n=== OVERALL STATS (London only, 2016-2025) ===")
